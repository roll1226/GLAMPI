...
@Component
export default class passwordAfterMypageForm extends Vue {
  public show1: boolean = false
  public show2: boolean = false
  // public password: string = ''
  // public passwordCheck: string = ''
  get passwordAfter() {
    return this.$store.state.mypage.passwordAfter
  }
  set passwordAfter(value: string) {
    this.$store.commit('registration/CHECK_PASSWORD_AFTER', value)
  }
  get passwordCheck() {
    return this.$store.state.mypage.passwordCheck
  }
  set passwordCheck(value: string) {
    this.$store.commit('mypage/CHECK_PASSWORDCHECK', value)
  }
  public rules: {} = {
    ...
    passwordCheck: (v: string) =>
      v === this.passwordAfter || 'パスワードが一致していません。'
  }
}
</script>
