...
export default class addressFacilityRegistration extends Vue {
  // public address: string = ''
  get address(): string {
    return this.$store.state.facilityRegist.address
  }

  set address(value: string) {
    this.$store.commit('facilityRegist/SET_ADDRESS', value)
  }
  code: object[] = []
  addres1: string = ''
  addres2: string = ''
    ...
    await this.$axios
      ...
      .then((res) => {
        if (res.data.code === 200) {
          ...
          this.$store.commit('facilityRegist/SET_STREET_ADDRESS', {
            pref,
            city,
            town
          })
